...
import (
	...
	"github.com/kterentev/wireguard-go/conn/bindtest"
	"github.com/kterentev/wireguard-go/tun"
	"github.com/kterentev/wireguard-go/tun/tuntest"
)
...
func genTestPair(tb testing.TB, realSocket bool) (pair testPair) {
	...
	for i := range pair {
		...
		p.dev = NewDevice(p.tun.TUN(), binds[i], NewLogger(level, fmt.Sprintf("dev%d: ", i)), false, false)
